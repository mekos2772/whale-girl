...
class AllowlistTests(unittest.TestCase):
    ...
    def test_candidates_rejected_retired_actions_cannot_be_loaded(self) -> None:
        ...
        for bad in ("hop", "jump_left", "jump_right", "hop_jump_left_12", "hop_jump_right_v1"):
            with self.assertRaises(KeyError, msg=bad):
                self.library.get(bad)
        # Candidate drag/vertical/click assets and old low-density IDs must
        # never enter the runtime path either. Their rebuilt formal IDs are
        ...
        for approved in (
            ...
            self.assertEqual(self.library.get(approved).canvas, (512, 768), approved)
